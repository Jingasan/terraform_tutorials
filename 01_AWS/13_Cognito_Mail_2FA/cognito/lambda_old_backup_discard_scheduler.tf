...
resource "aws_lambda_function" "lambda_old_backup_discard_scheduler" {
  ...
  # 関数名
  function_name = "${var.project_name}-old-backup-discard-scheduler-${local.lower_random_hex}"
...
resource "null_resource" "build_upload_lambda_old_backup_discard_scheduler" {
  ...
  # S3アップロード
  provisioner "local-exec" {
    command     = "aws s3 cp lambda.zip s3://${aws_s3_bucket.bucket_lambda.bucket}/old-backup-discard-scheduler/lambda.zip"
    working_dir = "lambda_old_backup_discard_scheduler"
  }
}
...
resource "null_resource" "update_lambda_old_backup_discard_scheduler" {
  ...
  # Lambda関数を更新
  provisioner "local-exec" {
    command     = "aws lambda update-function-code --function-name ${aws_lambda_function.lambda_old_backup_discard_scheduler.function_name} --s3-bucket ${aws_s3_bucket.bucket_lambda.bucket} --s3-key old-backup-discard-scheduler/lambda.zip --publish --no-cli-pager"
    working_dir = "lambda_old_backup_discard_scheduler"
  }
}
...
resource "aws_cloudwatch_event_rule" "lambda_old_backup_discard_scheduler" {
  ...
  schedule_expression = "cron(0 17 * * ? *)" # 毎日深夜2時に実行(AWS Backupが毎日深夜1時に実行される為)
  # 説明
  description = var.project_name
  # タグ
  tags = {
