...
resource "aws_cloudfront_distribution" "main" {
  ...
  origin_group {
    ...
    member {
      origin_id = var.cloudfront_origin_id_s3
    }
    member {
      origin_id = var.cloudfront_origin_id_lambda
    }
  }
  # オリジンの設定(S3)
  origin {
  ...
  default_cache_behavior {
    # オリジンID
    target_origin_id = var.cloudfront_origin_id_s3
    # HTTPはHTTPSにリダイレクトする
  ...
  ordered_cache_behavior {
    # オリジンID
    target_origin_id = var.cloudfront_origin_id_lambda
    # リバースプロキシ先のURL
    path_pattern = var.cloudfront_path_pattern_lambda
    # HTTPはHTTPSにリダイレクトする
    viewer_protocol_policy = "redirect-to-https"
    # 許可するHTTPメソッド
    allowed_methods = ["GET", "HEAD", "OPTIONS", "PUT", "POST", "PATCH", "DELETE"]
