...
resource "null_resource" "build_upload_lambda_verify_auth_challenge_response" {
  ...
  # S3アップロード
  provisioner "local-exec" {
    command     = "aws s3 cp lambda.zip s3://${aws_s3_bucket.bucket_lambda.bucket}/verify-auth-challenge-res/lambda.zip"
    working_dir = "lambda_verify_auth_challenge_response"
  }
}
...
resource "null_resource" "update_lambda_verify_auth_challenge_response" {
  ...
  # Lambda関数を更新
  provisioner "local-exec" {
    command     = "aws lambda update-function-code --function-name ${aws_lambda_function.lambda_verify_auth_challenge_response.function_name} --s3-bucket ${aws_s3_bucket.bucket_lambda.bucket} --s3-key verify-auth-challenge-res/lambda.zip --publish --no-cli-pager"
    working_dir = "lambda_verify_auth_challenge_response"
  }
}
