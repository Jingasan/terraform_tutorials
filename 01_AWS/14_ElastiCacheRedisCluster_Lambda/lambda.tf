...
resource "aws_lambda_function" "lambda" {
  ...
  environment {
    variables = {
      ...
      REDIS_PASSWORD = aws_elasticache_replication_group.redis.auth_token
    }
  }
...
resource "aws_lambda_function_url" "lambda" {
  ...
  # 認証タイプ(NONE:認証なし/AWS_IAM:認証されたIAMユーザーとロールのみによる関数コールを許可)
