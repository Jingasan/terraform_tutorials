...
# REST APIのエンドポイントリソース定義（/api/a/{proxy+}）
...
resource "aws_api_gateway_resource" "a_proxy" {
  ...
  # ここでは、/api/a/xxx, /api/a/yyy, /api/a/xxx/yyyなどのURLへのリクエストがこのエンドポイントにマッチするようになる。
  path_part = "{proxy+}"
}
...
# REST APIのエンドポイントリソース定義（/api/b/{proxy+}）
...
resource "aws_api_gateway_resource" "b_proxy" {
  ...
  # 受け付けるURLパスの設定
  # {proxy+}は任意のサブパスをまとめてすべてキャッチするワイルドカード
  # ここでは、/api/b/xxx, /api/b/yyy, /api/b/xxx/yyyなどのURLへのリクエストがこのエンドポイントにマッチするようになる。
  path_part = "{proxy+}"
}
