...
resource "aws_kinesis_firehose_delivery_stream" "lambda_cloudwatch_log_to_s3" {
  ...
  extended_s3_configuration {
    ...
    # S3オブジェクトのプレフィックス設定
    prefix = "logs/log_group=${aws_cloudwatch_log_group.lambda.name}/year=!{timestamp:yyyy}/month=!{timestamp:MM}/day=!{timestamp:dd}/hour=!{timestamp:HH}/"
    ...
    # バッファリング間隔（秒）の設定
    #   動的パーティション分割無効時：（default: 300, 最小: 0, 最大: 900）
    #   動的パーティション分割有効時：（default: 300, 最小: 60, 最大: 900）
    ...
    # ただし、間隔が短い値を設定したからといって即座に転送される訳ではなく、本設定はベストエフォート値である。
