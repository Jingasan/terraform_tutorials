...
resource "aws_s3_bucket" "bucket_lambda_create_auth_challenge" {
  # S3バケット名
  bucket = "${var.project_name}-lambda-create-auth-challenge-${local.lower_random_hex}"
...
resource "aws_lambda_function" "lambda_create_auth_challenge" {
  ...
  s3_bucket = aws_s3_bucket.bucket_lambda_create_auth_challenge.bucket
  s3_key    = "lambda.zip"
  # Lambda関数のタイムアウト時間
...
resource "null_resource" "build_upload_lambda_create_auth_challenge" {
  ...
  depends_on = [aws_s3_bucket.bucket_lambda_create_auth_challenge]
  # ソースコードに差分があった場合に実行
  triggers = {
    code_diff = join("", [
  ...
  # S3アップロード
  provisioner "local-exec" {
    command     = "aws s3 cp lambda.zip s3://${aws_s3_bucket.bucket_lambda_create_auth_challenge.bucket}/lambda.zip"
    working_dir = "lambda_create_auth_challenge"
  }
}
...
resource "null_resource" "update_lambda_create_auth_challenge" {
  ...
  # Lambda関数を更新
  provisioner "local-exec" {
    command     = "aws lambda update-function-code --function-name ${aws_lambda_function.lambda_create_auth_challenge.function_name} --s3-bucket ${aws_s3_bucket.bucket_lambda_create_auth_challenge.bucket} --s3-key lambda.zip --publish --no-cli-pager"
    working_dir = "lambda_create_auth_challenge"
  }
}
