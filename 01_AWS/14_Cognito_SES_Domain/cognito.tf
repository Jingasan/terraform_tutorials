...
resource "aws_cognito_user_pool" "user_pool" {
  ...
  email_configuration {
    ...
    # SESで認証するメールアドレスのARN
    source_arn = "arn:aws:ses:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:identity/${var.route53_domain}"
    # 送信元メールアドレスの設定
    from_email_address = "no-reply@${var.route53_domain}"
  }
  # ユーザーの属性情報更新の設定
