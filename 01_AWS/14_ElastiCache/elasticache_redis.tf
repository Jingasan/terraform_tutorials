...
# LambdaなどのElastiCacheに接続するアプリ用のセキュリティグループ
resource "aws_security_group" "elasticache_redis_app" {
  # セキュリティグループ名
  name = "${var.project_name}-elasticache-redis-app-sg"
  # セキュリティグループの説明
  description = "ElastiCache Redis Security Group for other app"
  # 適用先のVPC
  vpc_id = aws_vpc.default.id
  # タグ
  tags = {
...
resource "aws_security_group_rule" "elasticache_redis_ingress_app" {
  ...
  source_security_group_id = aws_security_group.elasticache_redis_app.id
  # 説明
...
# 設定エンドポイント(ElastiCache Redisの接続エンドポイント)
