...
resource "aws_s3_bucket" "bucket_lambda_verify_auth_challenge_response" {
  # S3バケット名
  bucket = "${var.project_name}-lambda-verify-auth-challenge-response-${local.lower_random_hex}"
...
resource "aws_lambda_function" "lambda_verify_auth_challenge_response" {
  ...
  # 関数名
  function_name = "${var.project_name}-verify-auth-challenge-response"
  ...
  s3_bucket = aws_s3_bucket.bucket_lambda_verify_auth_challenge_response.bucket
  s3_key    = "lambda.zip"
  # Lambda関数のタイムアウト時間
...
resource "null_resource" "build_upload_lambda_verify_auth_challenge_response" {
  ...
  depends_on = [aws_s3_bucket.bucket_lambda_verify_auth_challenge_response]
  # ソースコードに差分があった場合に実行
  triggers = {
    code_diff = join("", [
  ...
  # S3アップロード
  provisioner "local-exec" {
    command     = "aws s3 cp lambda.zip s3://${aws_s3_bucket.bucket_lambda_verify_auth_challenge_response.bucket}/lambda.zip"
    working_dir = "lambda_verify_auth_challenge_response"
  }
}
...
resource "null_resource" "update_lambda_verify_auth_challenge_response" {
  ...
  # Lambda関数を更新
  provisioner "local-exec" {
    command     = "aws lambda update-function-code --function-name ${aws_lambda_function.lambda_verify_auth_challenge_response.function_name} --s3-bucket ${aws_s3_bucket.bucket_lambda_verify_auth_challenge_response.bucket} --s3-key lambda.zip --publish --no-cli-pager"
    working_dir = "lambda_verify_auth_challenge_response"
  }
}
