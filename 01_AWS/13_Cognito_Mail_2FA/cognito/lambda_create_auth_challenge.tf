...
resource "aws_lambda_function" "lambda_create_auth_challenge" {
  ...
  # 関数名
  function_name = "${var.project_name}-create-auth-challenge-${local.lower_random_hex}"
...
resource "null_resource" "build_upload_lambda_create_auth_challenge" {
  ...
  # S3アップロード
  provisioner "local-exec" {
    command     = "aws s3 cp lambda.zip s3://${aws_s3_bucket.bucket_lambda.bucket}/create-auth-challenge/lambda.zip"
    working_dir = "lambda_create_auth_challenge"
  }
}
...
resource "null_resource" "update_lambda_create_auth_challenge" {
  ...
  # Lambda関数を更新
  provisioner "local-exec" {
    command     = "aws lambda update-function-code --function-name ${aws_lambda_function.lambda_create_auth_challenge.function_name} --s3-bucket ${aws_s3_bucket.bucket_lambda.bucket} --s3-key create-auth-challenge/lambda.zip --publish --no-cli-pager"
    working_dir = "lambda_create_auth_challenge"
  }
}
